...
%% general
    ...
    age      = scf_data(:,1);
    a2y_data = scf_data(:,5);     % a2y ratio from SCF 2019 data
...
disp("Starting 3b ----------")
    beta    = 0.96;
    a2y_mod = make_a2y(); % derive a2y from model
...
disp("Starting 3c ----------")
    ...
    fig = figure('Visible','off');
        plot(diff_struct.beta, diff_struct.dist, LineWidth=2);
        title('Beta and the Euclidean distance between SCF and model curves',FontSize=8);
        grid;
        saveas(fig,'beta_vs_dist.png');
    
    [val,id] = min(diff_struct.dist); % find minimum distance and its location
    last_beta = diff_struct.beta(id); % find optimal beta
    beta = last_beta;                 % assign optimal beta to global beta
    a2y_mod = make_a2y();             % a2y with optimal beta
